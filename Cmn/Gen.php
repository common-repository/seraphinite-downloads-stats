...
class Gen
{
	...
	static private function _SetArrField( &$arr, array $fieldPath, $val = null )
	{
		...
		if( $fld === '' )
			return( self::_SetArrField( $arr, $fieldPath, $val ) );
		...
		if( $isObj )
			return( self::_SetArrField( $arr -> { $fld }, $fieldPath, $val ) );
		return( self::_SetArrField( $arr[ $fld ], $fieldPath, $val ) );
	}
	...
	const CRYPT_METHOD_OPENSSL					= 'os2';
	const CRYPT_METHOD_MCRYPT					= 'mc2';
	const CRYPT_METHOD_XOR						= 'x2';
	...
	static function StrEncode( $str, $type = null )
	{
		...
		case self::CRYPT_METHOD_OPENSSL:
			...
			$key = openssl_digest( self::_StrEncDecSalt( 'perm_storage' ), 'SHA256', true );
		...
		case self::CRYPT_METHOD_MCRYPT:
			...
			$key = md5( self::_StrEncDecSalt( 'perm_storage' ), true );
		...
		case self::CRYPT_METHOD_XOR:
			...
			$str = self::XorData( $str, self::_StrEncDecSalt( 'perm_storage' ) );
			break;
	...
	static function StrDecode( $str )
	{
		...
		case self::CRYPT_METHOD_OPENSSL:
			...
			$key = openssl_digest( self::_StrEncDecSalt( $type === 'os' ? 'auth' : 'perm_storage' ), 'SHA256', true );
		...
		case self::CRYPT_METHOD_MCRYPT:
			...
			$key = md5( self::_StrEncDecSalt( $type === 'mc' ? 'auth' : 'perm_storage' ), true );
		...
		case self::CRYPT_METHOD_XOR:
			...
			$str = self::XorData( $str, self::_StrEncDecSalt( $type === 'x' ? 'auth' : 'perm_storage' ) );
			break;
	...
	static function FileOpenWithMakeDir( &$h, $filename, $mode, $use_include_path = false )
	{
		...
		if( @file_exists( $dir ) )
			return( @is_writable( $dir ) ? Gen::E_FAIL : Gen::E_ACCESS_DENIED );
	...
	static function StrStartsWith( string $haystack, string $needle )
	{
	...
	static function CloseCurRequestSessionForContinueBgWork()
	{
		...
		return( Gen::CallFunc( 'fastcgi_finish_request' ) );
	}
	...
	static function LogWrite( $file, $text, $severity = Gen::SevInfo )
	{
		switch( $severity )
		{
		...
		Gen::FileAddLine( $file, gmdate( 'd M Y H:i:s', time() ) . ' GMT <' . $severity . '>' . "\t" . $requestId . "\t" . $text );
	}
...
class LocId
{
	...
	static function UnPack( $v, $cbIdExpander )
	{
		...
		$ctx -> cb = function( $ctx, $v )
		{
			if( is_array( $v ) )
				return( array_map( array( $ctx, 'cb' ), $v ) );

			if( !is_string( $v ) || substr( $v, 0, 1 ) != "\x01" )
				return( $v );
...
class Lock
{
	...
	function Acquire()
	{
		...
		Gen::FileOpenWithMakeDir( $this -> h, $this -> file, 'c' );
		if( !$this -> h )
			return( false );
...
class Net
{
	...
	static function UrlDeParse( $urlComps, $flags = 0, $exclComps = array(  ), $inclComps = array() )
	{
		...
		$metas = array(
			...
			array( PHP_URL_QUERY,		'query',	array( '?',	'seraph_dlstat\\Net::UrlBuildQuery', '' ), array()	),
...
class HtmlNd
{
	...
	static function AddRemoveAttrClass( $nd, $valClasses, $valClassesRemove = '' )
	{
		...
		$nd -> setAttribute( 'class', implode( ' ', $val ) );
		return( true );
	}
...
class Wp
{
	...
	{
		return( @tempnam( Wp::GetTempDir(), substr( 'dlstat', 0, 3 ) ) );
	}

	static function _RemoteGet_RequestBefore( $url, $p1, $p2, $p3, &$options )
	{
	...
	static function RemoteGet( $url, $args = null )
	{
		...
		add_action( 'requests-requests.before_request', __CLASS__ . '::_RemoteGet_RequestBefore', 10, 5 );
		$res = wp_remote_get( $url, $args );
		remove_action( 'requests-requests.before_request', __CLASS__ . '::_RemoteGet_RequestBefore', 10 );
		return( $res );
	}
	...
	static function Loc_ScriptLoadEx( $handle, $domain, $ns, $pathAbsRoot )
	{
		...
		return( Wp::AddInlineScript( $handle, '(function(data){seraph_dlstat.Wp.Loc.SetData(data.locale_data.messages,"' . $domain . '");})(' . $translations . ')' ) );
	...
	static function Loc_ScriptLoad( $handle, $domain, $domainLocDir, $locSubPath )
	{
		...
		return( Wp::Loc_ScriptLoadEx( $handle, $domain, 'seraph_dlstat', $pathAbsRoot . '/' . $domainLocDir . '/' . $locSubPath ) );
	}
